...
public class GISDisplay {
	...
	public static GISDisplay getInstance() {
		if (instance == null)
			instance = new GISDisplay();

		return instance;
	}

	/**
	 * Singleton GISDisplay should be accessed by calling
	 * {@code GISDisplay.getInstance()}
	 */
	@Deprecated
	private GISDisplay() {
	...
	public void parseMapFile(File file) {
		try {
			...
			if (scanner.hasNextLine())
				scanner.nextLine(); // the first line is header information

			while (scanner.hasNextLine()) {
				String line = scanner.nextLine();
				String[] vals = line.replace(" ", "").split(","); // clear
																	// whitespace
				if (vals.length != 6)
					continue; // improperly formatted line
				...
				reaches.add(new Reach(id, sourceX, sourceY, sinkX, sinkY,
						nextID));
			}
	...
	private void sortReaches() {
		...
		for (int i = 0; i < reaches.size(); i++) {
			...
			for (int j = 0; j < reaches.size(); j++){
				if (reaches.get(j).getSourceX() == x && reaches.get(j).getSourceY() == y){
					reaches.get(i).setNextID(reaches.get(j).getReachID());
				}
			}
		}
	}
	...
	public BufferedImage getMapImage() {
		// If we haven't yet parsed the map data, don't attempt to create and
		// cache the map image
		if (!hasParsedMapData)
			return null;

		if (cachedMapData != null)
			return cachedMapData; // it's already created and cached, lets
									// return this

		// Let's create the buffered image to cache
		cachedMapData = new BufferedImage(Life.WINDOW_WIDTH * 2,
				Life.WINDOW_HEIGHT * 2, BufferedImage.TYPE_INT_ARGB);
		...
		for (Reach reach : reaches) {
			// TODO: Normalize the coordinates to the windowspace and manage
			// conversion to integers
			g2d.drawLine((int) reach.sourceX * 100, (int) reach.sourceY * 100,
					(int) reach.sinkX * 100, (int) reach.sinkY * 100);
		}
